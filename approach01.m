function [delaycost, proxy_a] = approach01(occur_t, handover_t, sim_t, m_a, a_b, ab_p, content_num)
%% initialize
...
if  ((occur_t-point_t(index_up))<m_b)   &&  ( (occur_t-point_t(index_up) )~=0 )
    delaycost=(content_num-count_down)*ab_p+(content_num-count_up+1)*m_b+(count_middle+1)*a_b
    proxy_a=(count_middle+1)
else
    delaycost=(content_num-count_down)*ab_p+(content_num-count_up)*m_b+count_middle*a_b
